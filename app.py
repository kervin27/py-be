...
def crea_utente():  # funzione che gestisce la creazione di un utente
    ...
    username = data.get("username")  # estrae il campo 'username' dal JSON della richiesta
    email = data.get("email")  # estrae il campo 'email' dal JSON della richiesta
    password = data.get("password")  # estrae il campo 'password' dal JSON della richiesta
    if not username or not email or not password:  # controlla che username, email e password siano forniti
        return jsonify({"error": "username, email e password obbligatori"}), 400  # se mancanti ritorna errore 400 con messaggio JSON
    success, msg = aggiungi_utente(username, email, password)  # chiama la funzione per aggiungere l'utente al DB
...
def ottieni_utenti():  # funzione che restituisce la lista degli utenti
    ...
    return jsonify([{"id": u[0], "username": u[1], "email": u[2]} for u in utenti])  # converte i risultati in lista di dizionari e restituisce JSON
